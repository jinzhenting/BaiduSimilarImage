    ...
    public partial class ImageAddForm : Form
    {
        ...
        private void addBack_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            for (int i = 0; i < datatable.Rows.Count; i++)// 遍历
            {
                ...
                string upPath = Regex.Replace(fullPath.Replace(api.Path, ""), @"^[\\]", "");// 记录时清除主目录，后面SQL转义
                ...
                if (selectDataTable.Rows.Count > 0)// 已有1条记录
                {
                    ...
                    else if (result == "Local")// 记录本地
                    {
                        ...
                        JObject localJson = ApiFunction.Up(client, fullPath, upPath, api.Tags1, api.Tags2);// 入库
                ...
                else
                {
                    ...
                    JObject json = ApiFunction.Up(client, fullPath, Regex.Replace(fullPath.Replace(api.Path, ""), @"^[\\]", ""), api.Tags1, api.Tags2);// 入库
